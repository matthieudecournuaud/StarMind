...
import { EntityArrayResponseType, IdeaService } from '../service/idea.service';
...
export class IdeaComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.ideas = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IIdea[] | null): IIdea[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const ideasNew = this.ideas ?? [];
      if (data) {
        for (const d of data) {
          if (ideasNew.some(op => op.id === d.id)) {
            ideasNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.ideaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
