...
import { EntityArrayResponseType, LikeHistoryService } from '../service/like-history.service';
...
export class LikeHistoryComponent implements OnInit {
  ...
  protected readonly likeHistoryService = inject(LikeHistoryService);
  ...
  trackId = (item: ILikeHistory): number => this.likeHistoryService.getLikeHistoryIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.likeHistories = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ILikeHistory[] | null): ILikeHistory[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const likeHistoriesNew = this.likeHistories ?? [];
      if (data) {
        for (const d of data) {
          if (likeHistoriesNew.some(op => op.id === d.id)) {
            likeHistoriesNew.push(d);
          }
        }
      }
      return likeHistoriesNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.likeHistoryService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
