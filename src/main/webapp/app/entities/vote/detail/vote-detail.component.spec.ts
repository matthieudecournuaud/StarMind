...
describe('Vote Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./vote-detail.component').then(m => m.VoteDetailComponent),
              resolve: { vote: () => of({ id: 123 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load vote on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', VoteDetailComponent);

      // THEN
      expect(instance.vote()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
