package com.nova.star.service.mapper;

import com.nova.star.domain.GlobalChat;
import com.nova.star.domain.User;
import com.nova.star.service.dto.GlobalChatDTO;
import com.nova.star.service.dto.UserDTO;
import java.util.Set;
import java.util.stream.Collectors;
import org.mapstruct.*;

/**
 * Mapper for the entity {@link GlobalChat} and its DTO {@link GlobalChatDTO}.
 */
@Mapper(componentModel = "spring")
public interface GlobalChatMapper extends EntityMapper<GlobalChatDTO, GlobalChat> {
    @Mapping(target = "users", source = "users", qualifiedByName = "userIdSet")
    GlobalChatDTO toDto(GlobalChat s);

    @Mapping(target = "removeUsers", ignore = true)
    GlobalChat toEntity(GlobalChatDTO globalChatDTO);

    @Named("userId")
    @BeanMapping(ignoreByDefault = true)
    @Mapping(target = "id", source = "id")
    UserDTO toDtoUserId(User user);

    @Named("userIdSet")
    default Set<UserDTO> toDtoUserIdSet(Set<User> user) {
        return user.stream().map(this::toDtoUserId).collect(Collectors.toSet());
    }
}
